...
def build_mnist_graph(number_of_classes):
    ...
    x_tensor = tf.reshape(x, graph_input_shape, name='image_reshape')

    x_normalized = tf.mul(x_tensor, 1/255, name='values_scaling')
    ...
    # layers
    h_conv1 = tf.nn.relu(conv2d(x_normalized, W_conv1) + b_conv1, name='conv1')
...
def train(images_path,
    ...
    #validate_data = load_pathes(validate_images_path)
    ...
    validate_batch_size = min(100, len(train_images_path))
    test_batch_size = min(20, len(train_images_path))
    n_epochs = 200000
    l_loss = list()
    best_loss = 0
    ...
    for epoch_i in xrange(n_epochs):
        ...
        # every 1000 do a full validation check
        if epoch_i % 1000 == 0:
            f_full_validation.write(str(loss))
            f_full_validation.flush()
